Shader "Silly/Projection/Decal Target"
{
    Properties
    {
        ...
        [Space(5)]
        [HDR] _TopLeftColor                             ("PlayerOneColor", Color)       = (1,1,1,1)
        [HDR] _TopRightColor                            ("PlayerTwoColor", Color)       = (1,1,1,1)
        [HDR] _BottomLeftColor                          ("PlayerThreeColor", Color)     = (1,1,1,1)
        [HDR] _BottomRightColor                         ("PlayerFourColor", Color)      = (1,1,1,1)

        [Toggle(USE_PLAYER_CIRCLE)]
        _UsePlayerInnerCircle("Use Player Inner Circle", Float) = 0
        [HDR] _PlayerColor                              ("PlayerColor", Color)          = (1,1,1,1)
        [NoScaleOffset] _BaseMap                        ("Outer Edge Texture", 2D)      = "white" {}
        [NoScaleOffset] _InnerCircleTexture             ("Inner Circle Texture", 2D)    = "white" {}
    ...
    SubShader
    {
        ...
        //  It is a decal!
            ...
            #pragma shader_feature USE_PLAYER_CIRCLE
            // -------------------------------------
            ...
            CBUFFER_START(UnityPerMaterial)
                half4 _TopLeftColor;
                half4 _TopRightColor;
                half4 _BottomLeftColor;
                half4 _BottomRightColor;
                half4 _PlayerColor;
...
#ifdef USE_PLAYER_CIRCLE
            Texture2D _InnerCircleTexture;
            SamplerState sampler_InnerCircleTexture;
#endif
            ...
            //Oliver - Calculate the color of the edge and give it its right color
                float2 texUV = positionOS.xz + float2(0.5, 0.5);

                float splitTopSide = step(texUV.y * -1.0f, -0.5f);
                float splitBottomSide = step(texUV.y, 0.5f);
                float splitLeftSide = step(texUV.x, 0.5f);
                float splitRightSide = step(texUV.x * -1.0f, -0.5f);

                half4 topLeftColor = _TopLeftColor * splitTopSide * splitLeftSide;
                half4 topRightColor = _TopRightColor * splitTopSide * splitRightSide;
                half4 bottomLeftColor = _BottomLeftColor * splitBottomSide * splitLeftSide;
                half4 bottomRightColor = _BottomRightColor * splitBottomSide * splitRightSide;

                half4 outerCircleColor = topLeftColor + topRightColor + bottomLeftColor + bottomRightColor;
                half4 outerSampleColor = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, texUV);
                half4 finalOuterCircleColor = outerSampleColor * outerCircleColor * outerSampleColor.a;

#ifdef USE_PLAYER_CIRCLE
                half4 innerSampleColor = SAMPLE_TEXTURE2D(_InnerCircleTexture, sampler_InnerCircleTexture, texUV);
                half4 playerCircleColor = innerSampleColor * _PlayerColor * innerSampleColor.a;

                half4 col = finalOuterCircleColor + playerCircleColor;
#else
                half4 col = finalOuterCircleColor;
#endif
